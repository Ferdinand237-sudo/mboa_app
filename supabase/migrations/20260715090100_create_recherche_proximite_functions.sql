-- Recherche de logements disponibles dans un rayon (km) autour d'un point (distance à vol d'oiseau, Haversine)
...
language sql stable as $$
  ...
  from (
    ...
    where l.statut = 'disponible' and l.lat is not null and l.lng is not null
  ) sub
  where sub.distance_km <= p_rayon_km
  order by sub.distance_km asc;
$$;

-- Recherche d'articles marketplace disponibles dans un rayon (km) autour d'un point
...
language sql stable as $$
  ...
  from (
    ...
    where a.statut = 'disponible' and a.lat is not null and a.lng is not null
  ) sub
  where sub.distance_km <= p_rayon_km
  order by sub.distance_km asc;
$$;

grant execute on function public.logements_proches(double precision, double precision, double precision) to anon, authenticated;
grant execute on function public.articles_proches(double precision, double precision, double precision) to anon, authenticated;
