...
serve(async (req) => {
  ...
  try {
    ...
    const { nom, email, password, whatsapp } = await req.json()
    ...
    const { data: authData, error: authError } =
      await supabaseAdmin.auth.admin.createUser({
        ...
        password,
        email_confirm: true,
        user_metadata: { nom, role: 'ambassadeur' },
      })
    ...
    // Mettre à jour le profil (la ligne public.users est créée par ailleurs
    // au moment de la création du compte Auth, comme pour create-vendor)
