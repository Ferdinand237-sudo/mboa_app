  ...
  return (
    <header className="sticky top-0 z-50 border-b border-mboa-border bg-mboa-card/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
        <Link href="/" data-tour="logo" className="flex items-center gap-2 shrink-0">
          ...
          <nav className="hidden items-center gap-1 md:flex">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                data-tour={`nav-${link.href.replace(/^\//, "")}`}
      ...
      {open && (
        <div data-tour-mobile-menu className={`border-t border-mboa-border bg-mboa-card px-4 py-3 ${menuSeulement ? "" : "md:hidden"}`}>
